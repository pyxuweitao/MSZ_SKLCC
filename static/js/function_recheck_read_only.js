...
window.onload = function () {
    ...
    if (window.location.search == '?history') {
        document.getElementById("returnback").setAttribute("href", '#');
        xmlhttp.open("GET", '/update_recheck_table_history?code=' + code + '&state=0', true);
        xmlhttp.send();
    } else {
        ...
        for (var j = 0; j < prog.length; j++) {
            if (j == 0) {
                ...
                if (record[i].getAttribute("state") == '1') {
                    ...
                    hs.innerHTML = '<select disabled="disabled"><option value = "1" selected = "selected"  >通过</option><option value = "0">不通过</option></select>';
                }
                else {
                    hs.innerHTML = '<select disabled="disabled"><option value = "1" >通过</option><option value = "0" selected = "selected" >不通过</option></select>';
                }
                line.appendChild(hs);
            }
            else {
                        ...
                        if (confirm("确定要修改数据吗？")) {
                            ...
                            if (parseInt(parent.innerHTML) != temp) {
                                if (parseInt(parent.innerHTML) || parseInt(parent.innerHTML) == 0) {
                                    ...
                                    if (gettotalcout(parent) > sample_count) {
                                        alert('数据有误');
                                        parent.click();
                                    } else {
