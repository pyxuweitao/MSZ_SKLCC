    ...
    for (var i = 0; i < record.length; i++) {
        ...
        for (var j = 0; j < prog.length; j++) {
            if (j == 0) {
                ...
                var content_id = record[i].getAttribute("id");
                link.href='/recheck/?content_id='+content_id+'&dept='+dept+'&inspectorno='+inspectorno+'&inspectorname='+inspectorname+'&batch='+batch+'&total_num='+record[i].getAttribute("total")+'&sample_num='+parseInt(record[i].getAttribute("sample"));
                link.innerHTML = '<br/>修改';
                
                var del_link = document.createElement('a');
                del_link.innerHTML = "删除";
                del_link.style.cursor = 'pointer';
                del_link.onclick = function(){
                    var re = new XMLHttpRequest();
                    var content_id = this.parentNode.parentNode.getAttribute("record_id");
                    re.open('GET','/delete_recheck_content/?content_id='+content_id+'&serialno='+window.location.hash.slice(1),false);
                    re.send();
                    window.location.reload(1);
                };
                hs.appendChild(link);
                hs.innerHTML +='/';
                hs.appendChild(del_link);
                line.appendChild(hs);
            }
            else {
                var line = document.createElement("tr");
