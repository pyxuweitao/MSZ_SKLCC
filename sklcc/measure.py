...
def toTypehorizon(res):
    ...
    for one in res:
        ...
        horizon_list[4].append(one['note'])
        for i, temp in enumerate(one['data']):
            horizon_list[i + 5].append(temp)
...
def get_measure_info_by_styleno_and_size(styleno, size):
    ...
    Raw.sql = "select partition, measure_res, common_difference, symmetry, note from sklcc_style_measure " \
              " WITH(NOLOCK) where styleno = '%s' and size = '%s' and measure_or_not = 1 and state = 1 order by serial" % (
              styleno, size)
    ...
    if target_list != False:
        for target in target_list:
            if target[0] not in [one['partition'] for one in res]:
                ...
                res.append({'partition': target[0], 'common_difference': '+' + target[2].replace('@', '/-'),
                            'symmetry': target[3], 'standard': target[1], 'note': target[4], 'data': []})
...
def get_measure_data_by_serialno(serialno, styleno, size):
    ...
    Raw.sql = "SELECT partition, measure_res, measure_type, symmetry1, symmetry2" \
              " FROM sklcc_measure_info WITH(NOLOCK) WHERE serialno = '%s' ORDER BY count_id, partition" % serialno
    target_list = Raw.query_all()
...
def get_inspector_measure_data(inspector_no, date, batch=None, size=None, contentid=None, measure_force_recheck=False,
    ...
    if contentid != None:
        ...
        Raw.sql = """SELECT partition, measure_res, symmetry1, symmetry2, count_id, measure_type
 				FROM sklcc_measure_record  a WITH(NOLOCK) JOIN sklcc_measure_info b WITH(NOLOCK)
        ...
        return res
        # 一检measure_in，二检的measure_force_recheck
    else:
        Raw.sql = """SELECT inspector_no, batch, a.styleno, a.size, partition, measure_res,
					symmetry1, symmetry2, count_id, measure_type, a.serialno FROM sklcc_measure_record a WITH(NOLOCK)
					 JOIN sklcc_measure_info b WITH(NOLOCK)
					ON a.serialno = b.serialno
					WHERE inspector_no = '%s' AND left( a.createtime, 10 ) = '%s' AND is_first_check = %d
					ORDER BY a.batch, a.size, %s count_id""" % (inspector_no, date,
                                                                0 if measure_force_recheck else 1,
                                                                'contentid, ' if measure_force_recheck else "")
...
def written_into_measure_info(info):
    ...
    if info['measure_type'] == 1:
        Raw.sql = "insert into sklcc_measure_info  WITH(ROWLOCK) ( serialno, styleno, createtime, partition," \
...
def written_into_measure_record(info, is_first_check='True'):
    Raw = Raw_sql()
    Raw.sql = "insert into sklcc_measure_record  WITH(ROWLOCK) ( serialno, createtime, inspector_no, inspector, batch, styleno, size, " \
...
def measure_commit(request):
    try:
        ...
        for one in data:
            batch = one['model']
            Raw.sql = "SELECT TOP 1 departmentno, styleno FROM producemaster WITH(NOLOCK) WHERE batch = '%s'" % batch
            target = Raw.query_one('MSZ')
            departmentno = ""
            styleno = "measure_commit_cannot_find_styleno"
            if target != False:
                departmentno = target[0]
                styleno = target[1].decode("gbk")
            size = one['size']
            ...
            if contentid == False:
                ...
                Raw.update()
                Raw.sql = "SELECT COUNT( DISTINCT contentid ) FROM sklcc_measure_info WITH(NOLOCK) WHERE serialno = '%s' " % serialno
            ...
            if len(res) != 0:
                for part in res:
                    ...
                    if len(data) == 1:
                        ...
                        SQL += "insert into sklcc_measure_info  WITH(ROWLOCK) ( serialno, styleno, createtime, partition," \
                               "measure_res, measure_type, count_id, contentid ) values( '%s', '%s', '%s', '%s', %f, %d, %d, '%s' );" \
                        ...
                        SQL += u"insert into sklcc_measure_info WITH(ROWLOCK) ( serialno, styleno, createtime, partition," \
                               u"measure_type, symmetry1, symmetry2, count_id, contentid ) values( '%s', '%s', '%s', '%s', %d, %f, %f, %d, '%s');" \
                               % (serialno, styleno, createtime, partition,
                                  1, symmetry1, symmetry2, count_id, contentid)
                if contentid != False:
                    # 二检插入sklcc_measure_record
                    if count_contentid == 0:
                        SQL += "insert into sklcc_measure_record WITH(ROWLOCK) ( serialno, createtime, inspector_no, inspector, batch, styleno, size, " \
                               "measure_count, state, is_first_check, departmentno ) values( '%s', '%s', '%s', '%s', '%s', '%s', '%s'" \
                               ", %d, 2, '%s', '%s' );" % (serialno, createtime, inspector_no,
                                                           inspector, batch, styleno, size,
                                                           count_id, 'False', departmentno)
                    else:
                        SQL += "UPDATE sklcc_measure_record WITH(ROWLOCK) SET measure_count = (" \
                               " SELECT COUNT( DISTINCT contentid ) FROM sklcc_measure_info WITH(NOLOCK) WHERE serialno = '%s' )" \
...
def measure_check(request):
    ...
    if 'deptno' in request.GET.keys():
        ...
        Raw.sql = "SELECT serialno, departmentno, left( createtime, 10 ), inspector_no, inspector, batch, size, measure_count FROM sklcc_measure_record" \
                  " WITH(NOLOCK) WHERE left( createtime, 10 ) >= '%s' AND left( createtime, 10 ) <= '%s' AND is_first_check = 1" % (
                  start, end)
        ...
        Raw.sql += ' ORDER BY left(createtime,10), inspector_no, departmentno,  batch, size, measure_count desc'
        ...
        if target_list != False:
            for i,target in enumerate(target_list):
                #to fix duplicated record bug
                if i > 0 and cmp( target[1:-1], target_list[i-1][1:-1] ) == 0:
                    continue
...
def measure_check_recheck(request):
    ...
    if 'deptno' in request.GET.keys():
        ...
        Raw.sql = "SELECT serialno, left( createtime, 10 ), inspector_no, inspector, batch, size, measure_count, departmentno FROM sklcc_measure_record" \
                  "  WITH(NOLOCK) WHERE left( createtime, 10 ) >= '%s' AND left( createtime, 10 ) <= '%s' AND is_first_check = 0" % (
                  start, end)
...
def get_batch_by_departmentno_and_date(request):
    ...
    Raw.sql = "SELECT DISTINCT batch FROM sklcc_measure_record WITH(NOLOCK) WHERE" \
              " left( createtime, 10 ) >= '%s' AND LEFT( createtime,10 ) <= '%s' AND is_first_check = %d" % (
              start, end, is_first_check)
    if departmentno != '':
        Raw.sql += " AND departmentno = '%s'" % departmentno
...
def measure_check_pass(request):
    ...
    Raw.sql = "update sklcc_measure_record WITH(ROWLOCK) set state = 1 where inspector_no = '%s' and left( createtime, 10 ) = '%s'" % (
    inspector_no, date)
    Raw.update()
    return HttpResponse()
...
def style_measure_check(request):
    ...
    if 'start' not in request.GET:
        Raw.sql = "SELECT DISTINCT styleno, size, left( createtime, 16 ) time, a.employeeno, b.employee, state, assessor_no, left( assesstime, 16 )" \
                  " FROM sklcc_style_measure a WITH(NOLOCK) JOIN sklcc_employee b WITH(NOLOCK) ON a.employeeno = b.employeeno" \
                  " WHERE state = 0 " \
                  " ORDER BY time desc"
    else:
        if bydate == 'on':
            ...
            Raw.sql = "SELECT DISTINCT styleno, size, left( createtime, 16 ) time, a.employeeno, b.employee, state, assessor_no, left( assesstime, 16 )" \
                      " FROM sklcc_style_measure a WITH(NOLOCK) JOIN sklcc_employee b WITH(NOLOCK) ON a.employeeno = b.employeeno" \
                      " WHERE LEFT( createtime, 10 ) >= '%s' AND LEFT( createtime, 10 ) <= '%s' ORDER BY time desc" % (
                      start, end)
        else:
...
def commit_style_measure(request):
    Raw = Raw_sql()
    styleno = request.GET['styleno']
    Raw.sql = "UPDATE sklcc_style_measure WITH(ROWLOCK) SET state = 0 WHERE styleno = '%s'" % styleno
    Raw.update()
    return HttpResponse()
...
def pass_style_measure(request):
    ...
    styleno = request.GET['styleno']
    styleno = urllib.unquote(str(styleno)).decode("utf-8")

    time = Current_time.get_accurate_time()
    Raw.sql = "UPDATE sklcc_style_measure WITH(ROWLOCK) SET state = %d, assessor_no = '%s', assesstime = '%s' WHERE styleno = '%s'" % (
...
def get_all_partition(measure_type_id):
    res = []
    Raw = Raw_sql()
    Raw.sql = "select partition from sklcc_measure_partition WITH(NOLOCK) where measure_type_id = '%s'" % measure_type_id
    target_list = Raw.query_all()
    if target_list != False:
        for target in target_list:
            res.append(target[0])
...
def style_measure(request):
    ...
    try:
        ...
        employeename = request.session['employee']
        Raw = Raw_sql()
        Raw.sql = "select measure_type_id, measure_type_name from sklcc_measure_type WITH(NOLOCK)"
        target_list = Raw.query_all()
        measure_list = []
        state = 0
        if target_list != False:
        ...
        common_size_list = []
        Raw.sql = "SELECT size, size_name FROM sklcc_measure_size WITH(NOLOCK) WHERE employeeno = '%s'" % em_number
        target_list = Raw.query_all()
        ...
        if 'style' in request.GET:
            ...
            Raw.sql = "select distinct partition, serial, a.employeeno, b.employee, state, assessor_no, left( assesstime, 16 )" \
                      ...
                      " a WITH(NOLOCK) JOIN sklcc_employee b WITH(NOLOCK) ON a.employeeno = b.employeeno where styleno = '%s' ORDER BY serial" % style
            target_list = Raw.query_all()
            if target_list == False:
                state = 1
                is_mine = 1
                ...
                for target in target_list:
                    ...
                    Raw.sql = "select distinct common_difference, symmetry, size, partition, measure_res, measure_or_not, note, size_serial" \
                              " from sklcc_style_measure WITH(NOLOCK) where styleno = '%s' and partition = '%s' order by size_serial" % (
...
def written_into_style_measure(record):
    ...
    T = Current_time()
    now = T.time_str
    Raw.sql = u"insert into sklcc_style_measure WITH(ROWLOCK) ( state, employeeno,serial,styleno, common_difference, createtime, symmetry, size, partition, " \
              u"measure_res, note, size_serial, measure_or_not  ) values( 2, '%s', '%d', '%s', '%s', '%s', %f, '%s', '%s', %f, '%s', %d, " % (
                  record['employeeno'], record['index'], record['styleno'], record['common_difference'], now,
                  record['symmetry'], record['size'],
...
def submit_style_measure(request):
    try:
        ...
        if res['new_style'] != res['old_style']:
            Raw.sql = "select * from sklcc_style_measure WITH(NOLOCK)  where styleno = '%s'" % res['new_style']
            target = Raw.query_one()
            if target != False:
