...
def find_employeeno(username):
    Raw = Raw_sql()
    Raw.sql = "select employeeno from sklcc_employee WITH(NOLOCK) where username = '%s'" % username
...
def find_em_name(em_number):
    Raw = Raw_sql()
    Raw.sql = "select employee from sklcc_employee WITH(NOLOCK) where employeeno = '%s'" % em_number
    target = Raw.query_one()
    return target[0] if target != False else None
...
def find_department_authority_user(username, authorityid):
    ...
    """
    Raw = Raw_sql()
    Raw.sql = "select DISTINCT departmentno from sklcc_employee_authority WITH(NOLOCK) where username = '%s' and authorityid = %d order by departmentno" % (
    username, int(authorityid))
...
def find_inspector_by_department_authority(departmentno, authorityid):
    Raw = Raw_sql()
    Raw.sql = "select distinct username from sklcc_employee_authority WITH(NOLOCK) where departmentno = '%s' and authorityid = '%d'" % (
    departmentno, authorityid)
    target_list = Raw.query_all()
    user_list = []
    if target_list != False:
        for target in target_list:
            user_list.append({'username': target[0], 'employeeno': find_employeeno(target[0]),
                              'employee': find_em_name(find_employeeno(target[0]))})
    return user_list


def find_department_name(departmentno):
    Raw = Raw_sql()
    Raw.sql = "select distinct department from sklcc_department WITH(NOLOCK) where departmentno = '%s'" % departmentno
    target = Raw.query_one()
    return target[0]
...
def get_all_employee():
    Raw = Raw_sql()
    Raw.sql = "select employee, employeeno from sklcc_employee WITH(NOLOCK)"
    target_list = Raw.query_all()
    employee_list = []
    if target_list != False:
        for target in target_list:
...
def get_record_totalnumber(serialno):
    Raw = Raw_sql()
    Raw.sql = "select distinct barcode, number_pack from sklcc_info WITH(NOLOCK) where serialno = '%s'" % serialno
...
def get_record_totalreturn(serialno):
    Raw = Raw_sql()
    Raw.sql = "select returnno from sklcc_table WITH(NOLOCK) where serialno = '%s'" % serialno
...
def get_styleno_by_batch(batch):
    ...
    """
    Raw = Raw_sql()
    Raw.sql = u"SELECT TOP 1 styleno FROM producemaster WITH(NOLOCK) WHERE batch = '%s' and Formstate='审核'" % batch

    styleno = Raw.query_one('MSZ')[0].decode("GB18030")
    if styleno != False:
        return styleno
    else:
        return "NULL"
...
def get_all_barcode_by_serialno(serialno):
    Raw = Raw_sql()
    Raw.sql = '''SELECT barcode, id FROM SKLCC_INFO WITH(NOLOCK) WHERE SERIALNO = '%s'
			AND id IN ( SELECT MIN(ID) FROM SKLCC_INFO WITH(NOLOCK) WHERE SERIALNO = '%s' GROUP BY barcode )
		''' % (serialno, serialno)
    target_list = Raw.query_all()
    if target_list != False:
...
def get_bald_totalnumber(recheckor_no, date):
    ...
    Raw.sql = '''select TOP 1 totalnumber
    						from sklcc_recheck_bald WITH(NOLOCK) where left(createtime,10)='%s' and recheckor_no = '%s'
    				''' % (date, recheckor_no)
...
def transStringEncodedByURL(string):
    reload(sys)
    sys.setdefaultencoding("utf-8")
    return unquote(str(string)).decode("utf-8")
