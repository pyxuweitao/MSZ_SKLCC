...
def written_into_sklcc_effciency(recheckor_no='NoBody', recheckor='NoBody', table_date='NoDate', specified_date=''):
    '''
	用在三处
	@1用于定时插入抽验员效率数据
	@2用于当抽验员提交退回数据之后，修正当天的总工时，因为抽验员很有可能修改了当天的总检验件数
	@3用于需要修正检验员
	:param recheckor_no:抽验员工号
	:param recheckor:抽验员名
	:param table_date:抽验员修改的表单的创建时间
	'''
    Raw = Raw_sql()
    real_work_time_dict = {}
    recheckor_list = []
    if recheckor_no == 'NoBody':
        ...
        T = Current_time()
        date = T.set_date_yesterday().split(" ")[0] if specified_date == '' else specified_date
        Raw.sql = "SELECT employeeno, real_work_time FROM sklcc_effciency WHERE date = '%s'" % date
        target_list = Raw.query_all()
        if target_list != False:
            real_work_time_dict = dict((target for target in target_list))

        Raw.sql = "DELETE FROM sklcc_effciency WHERE date = '%s'" % date
        Raw.update()
        ...
        if target_list != False:
            for target in target_list:
                ...
                if employeeno not in real_work_time_dict.keys():
                    real_work_time_dict[employeeno] = real_work_time
    else:
        # 抽验员修改退回表单
        recheckor_list.append({'employeeno': recheckor_no, 'employee': recheckor})
        date = table_date
        Raw.sql = "SELECT real_work_time FROM sklcc_effciency WHERE employeeno = '%s' and date = '%s'" % (
        recheckor_no, date)
        target = Raw.query_one()

        if target != False:
            real_work_time = target[0]
        else:
            Raw.sql = "select Top 1 form7_real_work_time from sklcc_config"
            target = Raw.query_one()
            real_work_time = target[0] if target != False and target != None else 0
        Raw.sql = "DELETE FROM sklcc_effciency WHERE employeeno = '%s' and date = '%s'" % (recheckor_no, date)
        Raw.update()
    Raw.sql = '''SELECT recheckor_no, recheckor, sum( total * slowtime )  FROM
				( SELECT recheckor_no, recheckor, SUM(samplenumber) total, batch FROM
				( select distinct recheckor, recheckor_no, batch, samplenumber, contentid from sklcc_recheck_info a
				join sklcc_recheck_content b on a.serialno = b.serialno
				where left( a.createtime, 10 ) = '%s' %s
				  ) recheck GROUP BY recheckor_no, recheckor, batch ) recheckor_work_num JOIN
				 ( select distinct batch, slowtime from [BDDMS_MSZ].[dbo].ProduceMaster, [BDDMS_MSZ].[dbo].ProduceStyle
					where [BDDMS_MSZ].[dbo].ProduceStyle.producemasterid = [BDDMS_MSZ].[dbo].ProduceMaster.producemasterid and qcbz = 1
				) produce ON produce.batch = recheckor_work_num.batch
				 group by recheckor_no, recheckor''' % (
    date, "and recheckor_no = '%s'" % recheckor_no if recheckor != 'NoBody' else '')

    res_list = Raw.query_all()
    recheckor_no_list = [employee['employeeno'] for employee in recheckor_list]
    if res_list != False:
        for res in res_list:
            recheckor_no = res[0]
            if recheckor_no not in recheckor_no_list:
                continue
            Raw.sql = u"""SELECT SUM(samplenumber) * ( SELECT dbo.get_worktime_by_batch_and_checktype( batch, '套灯模' ) ), batch FROM(
								SELECT DISTINCT contentid, samplenumber, a.batch FROM sklcc_recheck_info a JOIN sklcc_recheck_content b
								ON a.serialno = b.serialno
								WHERE recheckor_no = '%s' AND left( a.createtime, 10 ) = '%s' ) samplesum
								GROUP BY batch""" % (recheckor_no, date)
            ...
            Raw.sql = "insert into sklcc_effciency( employeeno, employee, date, work_time, real_work_time, effciency )" \
                      "values( '%s', '%s', '%s', %f, %f, %f )" % (
                          recheckor_no, recheckor, date, work_time, real_work_time_dict[recheckor_no],
                          round(float(work_time) / float(real_work_time_dict[recheckor_no]) if real_work_time_dict[
                                                                                                   recheckor_no] != 0 else 0,
                                4))
            Raw.update()
    ...
    if res_list != False:
        ...
        for one in recheckor_list:
            ...
            bald_total_slowtime = get_bald_totalnumber(recheckor_no, date) * get_bald_slowtime()
            work_time += bald_total_slowtime
            Raw.sql = "insert into sklcc_effciency WITH(ROWLOCK)( employeeno, employee, date, work_time, real_work_time, effciency )" \
                      "values( '%s', '%s', '%s', %f, %f, %f )" % (
                          recheckor_no, recheckor, date, work_time, real_work_time_dict[recheckor_no],
                          round(float(work_time) / float(real_work_time_dict[recheckor_no]) if real_work_time_dict[
                                                                                                   recheckor_no] != 0 else 0,
...
def get_info(barcode_num, inspector_no):
    ...
    Raw.sql = "SELECT b.inspector_no, b.state, a.dirty FROM [SKLCC].DBO.sklcc_info a WITH(NOLOCK) JOIN [SKLCC].DBO.sklcc_record b" \
    ...
    if target_list != False:
        for target in target_list:
            if target[2] == True:
                return dict(state=7) #表示存在脏数据，不允许检验员重新扫描
        if target_list[0][0] != inspector_no:
            return dict(state=6)
        else:
            if target_list[0][1] == 2:
                state = 2
            else:
                state = 3
    else:
    ...
    Raw.sql = """SELECT TOP 1 a.Batch, a.DepartmentNo, a.number, a.styleno, b.packid, b.size, b.number, a.producemasterid
	 FROM ProduceMaster a WITH(NOLOCK) JOIN producepack b WITH (NOLOCK) ON a.producemasterid = b.producemasterid
	  WHERE b.PackTXM = '%s' AND FormState = '审核'""" % barcode_num
    ...
    styleno      = target_mas[3].decode("GB18030")
    packid       = target_mas[4]
    size         = target_mas[5]
...
def get_all_check_type():
    Raw = Raw_sql()
    Raw.sql = "select check_id, check_type from sklcc_check_type WITH(NOLOCK) where check_id != '2'"
    check_type_list = []
    target_list = Raw.query_all()
    if target_list != False:
        for target in target_list:
        ...
        if size in [target[0].strip() for target in target_list]:
            ...
            if partition_list != False:
                ...
                for partition in partition_list:
                    xml += """<measure partition = "%s" common_difference = "%s" symmetry = "%.2f" measure_res = "%.2f" note = "%s"/>""" % (
                        partition[0], '+' + partition[1].replace('@', '/-'), partition[2], partition[3], partition[4])
                xml += """</ME>"""
...
def update_info(request):
    try:
        ...
        if info_ini['state'] == 7:#脏数据，不允许重刷
            xml += """<state value = "7"></state></info></xml>"""
            return HttpResponse(xml)
        if info_ini['state'] == 4:
        ...
        Raw.sql = "select a.worklineno, a.workname, b.employee, b.employeeno from producestyle a WITH (NOLOCK) left" \
                  " join producepackproc b WITH (NOLOCK) on a.worklineno = b.worklineno " \
                  "where a.producemasterid = '%s' and b.packTXM = '%s' and a.FlowInfoID<>'CF0E3C88-DB2F-4608-888B-5A19BA6FA7DC'" \
                  " order by a.worklineno" % (info_ini['masterid'], barcode)
        ...
        if info_ini['state'] == 2:
            ...
            Raw.sql = "select a.serialno, a.employeeno, a.workname, a.workno, a.questionname, a.questionno, a.returnno," \
                      ...
                      " where barcode = '%s' and a.batch = '%s' and b.inspector_no = '%s'" % (
                          barcode, info_ini['batch'], inspector_no)
            res_list = Raw.query_all()
            ...
            if res_list[0][6] == 0: #如果有一个returnno是0，那么这个条码只有一个记录，且returnno=0
                ...
                Raw.sql += u"update sklcc_record WITH(ROWLOCK)" \
                           u" set totalnumber = dbo.get_totalnumber_by_serialno( '%s' )" \
                           u" WHERE serialno = '%s';" % (serialno, serialno)
                Raw.update()
                return HttpResponse(xml)

            returnno_sum = 0
            for info_one in res_list:
                returnno_sum += info_one[6] #这里的returnno_sum都是记录单未修改的条码，所以可以放心的加和
                ...
                Raw.sql = "select returnno from sklcc_table WITH(NOLOCK) where serialno = '%s'" \
                          " and employeeno = '%s' and questionno = %d and workno = %d" % (
                    info_one[0], info_one[1], info_one[5], info_one[3])
                ...
                if returnno == 0:
                    ...
                    Raw.sql = "update sklcc_table WITH(ROWLOCK) set returnno = %d where serialno = '%s'" \
                              " and employeeno = '%s' and questionno = %d and workno = %d" % (
                        returnno, info_one[0], info_one[1], info_one[5], info_one[3])
                    Raw.update()

                    # update record table
            #如果不是在上一个if中return的话还要处理sklcc_record的数据
...
def written_info_in_database(serialno="", barcode="", check_id=1, batch="", departmentno="", workno=0,
    ...
    if request != None:
        if barcode != 'null':
            try:
                Raw.sql = "insert into sklcc_info WITH(ROWLOCK) ( state, returnno, barcode, boxno, batch, number_pack, serialno ) values( %d, %d, '%s', %d, '%s', %d, '%s' )" % (
    ...
    if workno == 0:
        ...
        Raw.sql = "SELECT WorkName from ProducePackProc WITH (NOLOCK) where PackTXM = '%s' and WorkLineNo = %d" % (
        barcode, int(workno))
    ...
    Raw.sql = "insert into sklcc_info WITH(ROWLOCK) values( %d, '%s',%d, %d, '%s', %d, '%s', '%s', %d, '%s', %d, %d, '%s', '%s'  )" % (
...
def written_table_in_database(temp):
    ...
    sys.setdefaultencoding('utf-8')
    table = Raw_sql()
    table.sql = u"select returnno from sklcc_table WITH(NOLOCK) where serialno = '%s' and employeeno = '%s' and workno = '%s' and questionno = %d" % (
    ...
    if target == False:
        try:
            ...
            table.sql = "insert into sklcc_table WITH(ROWLOCK) values ( '%s', %d, '%s', '%s', '%s', %d, %d, %d, '%s', %d )" % (
                serialno, state, employeeno, employee, workname, workno, returnno, questionno, questionname,
                questiontype)
            # print table.sql
            table.update()
        except Exception, e:
        ...
        table.sql = "update sklcc_table WITH(ROWLOCK) set returnno = %d where serialno = '%s' and employeeno = '%s' and workno = %d and questionno = %d" % (
...
def written_record_in_database(temp):
    ...
    Raw.sql = "select * from sklcc_record WITH(NOLOCK) where serialno = '%s'" % temp.serialno
    target = Raw.query_one()
    ...
    if target == False:
        ...
        Raw.sql = "insert into sklcc_record WITH(ROWLOCK) ( serialno, createtime, inspector, inspector_no, state, totalnumber, departmentno, batch, totalreturn, check_id, check_type )" \
                  " values( '%s', '%s', '%s', '%s', %d, %d, '%s', '%s', %d, '%s', '%s' )" % (
                      serialno, createtime, inspector, inspector_no, state, totalnumber, departmentno, batch,
                      totalreturn,
        ...
        totalnumber = get_record_totalnumber(temp.serialno)
        totalreturn = get_record_totalreturn(temp.serialno)
        Raw.sql = "update sklcc_record WITH(ROWLOCK) set totalnumber = %d, totalreturn = %d where serialno = '%s'" % (
...
def written_return_check_in_datebase(serialno):
	...
	"""
    Raw = Raw_sql()
    Raw.sql = "select * from sklcc_return_check WITH(NOLOCK) where serialno = '%s'" % serialno
    target = Raw.query_all()
    if target == False:
        Raw.sql = "insert into sklcc_return_check WITH(ROWLOCK) ( serialno, real_return, ok, state, inner_serialno, questionno )" \
...
def get_return_percentage(departmentno, date):
    try:
        Raw = Raw_sql()
        Raw.sql = "select sum( totalreturn ), sum( totalnumber ) from sklcc_record WITH(NOLOCK) where left( createtime, 10 ) = '%s' and departmentno = '%s'" % (
            date, departmentno)
        totalnumber = 0
        totalreturn = 0
        target = Raw.query_one()
...
def get_common_difference(styleno, partition):
    Raw = Raw_sql()
    Raw.sql = "select common_difference from sklcc_style_measure WITH(NOLOCK) where styleno = '%s' and partition = '%s' and state = 1" % (
        styleno, partition)
    target = Raw.query_one()
...
def get_symmetry(styleno, partition):
    Raw = Raw_sql()
    Raw.sql = "select symmetry from sklcc_style_measure WITH(NOLOCK) where styleno = '%s' and partition = '%s' and state = 1" % (
        styleno, partition)
    target = Raw.query_one()
...
def get_measure_res(styleno, partition):
    Raw = Raw_sql()
    Raw.sql = "select measure_res from sklcc_style_measure WITH(NOLOCK) where styleno = '%s' and partition = '%s' and state = 1" % (
        styleno, partition)
    target = Raw.query_one()
...
def commit_res(request):
    ...
    try:
        ...
        Raw.sql = u"select serialno from sklcc_record WITH (NOLOCK) where state = 2 and batch = '%s' and inspector_no = '%s' and" \
        ...
        SQL += u"DELETE FROM SKLCC_INFO WITH(ROWLOCK) WHERE BARCODE = '%s';"%insert_info['barcode']
        baoshu_barcode = unicode(uuid.uuid1())
        #baoshu_barcode = 'baoshu'
        if len(json) != 0:
            for item in json:
                # 疵点录入
                if insert_info['barcode'] != 'null':
                    SQL += u"""insert into sklcc_info WITH(ROWLOCK) ( state, questionname, questionno, type, workname, workno, employee,
                    ...
                    # 报数录入 其中的barcode由一个单独生成的uuid代替
                else:
                    SQL += u"""insert into sklcc_info WITH(ROWLOCK) ( state, questionname, questionno, type, workno, workname, employee,
 	    					employeeno, returnno, number_pack, serialno, batch, barcode, boxno )
 	    					values( 2, '%s',%d, [BDDMS_MSZ].DBO.get_questiontype_by_no( %d ),
 	    			%d, '%s','%s', '%s', %d, %d, '%s', '%s', '%s', 0); """ % (
                        item['mistake_name'], int(item['mistake_no']), int(item['mistake_no']), int(item['work_no']),
                        'unknown',
                        item['employee_name'], item['employee_no'], int(item['count']), insert_info['number_pack'],
                        insert_info['serialno'], insert_info['batch'], baoshu_barcode)
                totalreturn += int(item['count'])
                # SQL = SQL[:-10] + ';' #去除最后一个UNION ALL 并且加上执行分号
        else:
            if insert_info['barcode'] != 'null':
                SQL = u"insert into sklcc_info WITH(ROWLOCK) ( state, returnno, barcode, boxno, batch, number_pack, serialno )" \
                ...
                SQL = u"insert into sklcc_info WITH(ROWLOCK) ( state, returnno, barcode, boxno, batch, number_pack, serialno)" \
                      u" values( 2, 0, '%s', 0, '%s', %d, '%s' );" % (
                          baoshu_barcode, insert_info['batch'], insert_info['number_pack'], insert_info['serialno'])
        ####    ###################################        sklcc info table     ####################################################

        ####    ###################################        sklcc record table   ####################################################
        if is_serialno_new:
            SQL += u"insert into sklcc_record WITH(ROWLOCK) ( serialno, createtime, inspector, inspector_no, state, totalnumber," \
            ...
            SQL += u"insert into sklcc_return_check WITH(ROWLOCK) ( serialno, real_return, ok, state, inner_serialno, questionno )" \
                   " values( '%s',  %d, %d, %d, '%s', %d )" % (
                       insert_info['serialno'], 0, 1, 0, unicode(uuid.uuid1()), 0)
        else:
            SQL += u"update sklcc_record WITH(ROWLOCK) set totalnumber = dbo.get_totalnumber_by_serialno( '%s' )," \
                   u" totalreturn = (SELECT COALESCE(SUM( returnno ),0) FROM sklcc_table WITH(NOLOCK) WHERE serialno =  '%s' ) " \
                   u"where serialno = '%s';" % (
...
def update_all_state_to(serialno, state, is_measure=False):
    Raw = Raw_sql()
    if is_measure == False:
        Raw.sql = "select * from sklcc_record WITH(NOLOCK) where serialno = '%s'" % serialno
        target_record = Raw.query_one()

        if target_record == False:
            return False
        else:
            Raw.sql = "update sklcc_record WITH(ROWLOCK) set state = %d where serialno = '%s'" % (state, serialno)
            Raw.update()

        Raw.sql = "select * from sklcc_table WITH(NOLOCK) where serialno = '%s'" % serialno
        target_table = Raw.query_all()
        if target_table == False:
            return False
        else:
            Raw.sql = "update sklcc_table WITH(ROWLOCK) set state = %d where serialno = '%s'" % (state, serialno)
            Raw.update()

        Raw.sql = "select TOP 1 * from sklcc_info WITH(NOLOCK) where serialno = '%s'" % serialno
        target_info = Raw.query_all()
        if target_info == False:
            return False
        else:
            Raw.sql = "update sklcc_info WITH(ROWLOCK) set state = %d where serialno = '%s'" % (state, serialno)
            Raw.update()
    else:
        Raw.sql = "update sklcc_measure_record WITH(ROWLOCK) set state = %d where serialno = '%s'" % (state, serialno)
        Raw.update()


def find_table_with_serialno(serialno):
    table = Raw_sql()
    table.sql = "select * from sklcc_table WITH(NOLOCK) where serialno = '%s'" % serialno
...
def find_record_state_is(state, employeeno, authority_in_use, control):
    ...
    if authority_in_use == 2:
        ...
        Raw.sql = '''SELECT check_type, SUBSTRING(createtime,6,11) time, state, batch,
					...
					( SELECT COUNT(*) FROM sklcc_info WITH(NOLOCK) WHERE type = 0 AND serialno = record.serialno ) AS weak,
					( SELECT COUNT(*) FROM sklcc_info WITH(NOLOCK) WHERE type = 1 AND serialno = record.serialno ) AS bad,
					( SELECT COUNT(*) FROM sklcc_info WITH(NOLOCK) WHERE type = 2 AND serialno = record.serialno ) AS strong
					FROM sklcc_record record  WITH(NOLOCK)
        ...
        Raw.sql = '''SELECT check_type, SUBSTRING(createtime,6,11) time, state, batch,
					...
					AND departmentno IN
          			( SELECT departmentno FROM sklcc_employee_authority WITH(NOLOCK)
...
def find_record_by_date(start, end, employeeno, authority_in_use):
    ...
    if authority_in_use == 2:
        ...
        Raw.sql = '''SELECT check_type, SUBSTRING(createtime,6,11) time, state, batch,
					...
					( SELECT COUNT(*) FROM sklcc_info WITH(NOLOCK) WHERE type = 2 AND serialno = record.serialno ) AS strong
					FROM sklcc_record record WITH(NOLOCK)
					WHERE inspector_no = '%s' AND createtime >= '%s' AND createtime,10 <= '%s' AND state = 2
        ...
        Raw.sql = '''SELECT check_type, SUBSTRING(createtime,6,11) time, state, batch,
					...
					( SELECT COUNT(*) FROM sklcc_info WITH(NOLOCK) WHERE type = 2 AND serialno = record.serialno ) AS strong
					FROM sklcc_record record WITH(NOLOCK)
					WHERE createtime >= '%s' AND createtime <= '%s' AND state < 2
					AND departmentno IN
          			( SELECT departmentno FROM sklcc_employee_authority WITH(NOLOCK)
            			WHERE authorityid = 4 AND username = '%s' )
...
def sync_table_from_record(serialno):
    ...
    Raw = Raw_sql()
    Raw.sql = "update sklcc_record WITH(ROWLOCK) set totalreturn = %d where serialno = '%s'" % (totalreturn, serialno)
    Raw.update()


def count_type(serialno):
    SQL = Raw_sql()
    SQL.sql = "select questiontype, returnno from sklcc_table WITH(NOLOCK) where serialno = '%s'" % serialno
...
def update_table_check(request):
    ...
    Raw.sql = "select departmentno, totalnumber, createtime, batch, inspector," \
              " inspector_no, check_type, totalreturn, serialno from sklcc_record WITH(NOLOCK) where serialno = '%s'" \
        ...
        try:
            ...
            if state == '0':
                ...
                for record in record_list:
                    ...
                    #在sklcc_table中查找来判定要修改的记录是否是新纪录
                    Raw.sql      = "select * from sklcc_table WITH(NOLOCK) where serialno = '%s' and workno = %d and questionno = %d and employeeno = '%s'" % (
                    ...
                    if target == False:
                        if count == 0:
                            ...
                            Raw.sql = "select state from sklcc_table WITH(NOLOCK) where serialno = '%s' and workno = %d" % (
                                serialno, workno)
                            target = Raw.query_one()
                            #插入新记录单记录不会对sklcc_info造成影响
                            Raw.sql = "insert into sklcc_table WITH(ROWLOCK) values ( '%s', %d, '%s', '%s', '%s', %d, %d, %d, '%s', %d )" % (
                        ...
                        if count == 0:
                            ...
                            Raw.sql = "update sklcc_table WITH(ROWLOCK) set returnno = %d where serialno = '%s' and workno = %d and questionno = %d and employeeno = '%s';" % (
                                count, serialno, workno, questionno, employeeno)

                        Raw.sql += "UPDATE sklcc_info SET dirty = 1 where serialno = '%s' and workno = %d and questionno = %d and employeeno = '%s';" % (
                                serialno, workno, questionno, employeeno)
                        Raw.update()
...
def pass_table(request):
    ...
    T = Current_time()
    if is_measure == '0':
        Raw.sql = "update sklcc_record WITH(ROWLOCK) set assessor = '%s', assessor_no = '%s',assesstime = '%s' where serialno = '%s'" % (
            request.session['employee'], request.session['employeeno'], T.time_str, serialno)
        Raw.update()
    else:
        Raw.sql = "update sklcc_measure_record WITH(ROWLOCK) set assessor = '%s', assessor_no = '%s', assesstime = '%s' where serialno = '%s'" % (
            request.session['employee'], request.session['employeeno'], T.time_str, serialno)
        Raw.update()
    return HttpResponse()
...
def submit_partion_set(request):
    ...
    Raw.sql = "delete from sklcc_measure_type WITH(ROWLOCK) where measure_type_id = '%s'" % id
    Raw.update()
    Raw.sql = "insert into sklcc_measure_type WITH(ROWLOCK) values( '%s', '%s' )" % (id, result[0]['type_name'])
    Raw.update()

    for one in result[0]['partition_list']:
        Raw.sql = "insert into sklcc_measure_partition WITH(ROWLOCK) WITH(ROWLOCK) ( measure_type_id, partition ) values( '%s', '%s' )" % (
        id, one)
        Raw.update()
    ...
    for size_set in json:
        ...
        for size in size_set['value']:
            if size == '':
                ...
                size_list += (size + "@")
        Raw.sql += "INSERT INTO sklcc_measure_size WITH(ROWLOCK) ( createtime, employeeno, size_name, size )" \
    ...
    try:
        ...
        if type == "1":
            ...
            for batch in batch_list:
                ...
                for day in distance:
                    Raw.sql = r"select totalreturn from sklcc_record where batch = '%s'" \
                              r" and createtime like '%s%%%%'" % (
                                  batch, day)
                    target_list = Raw.query_all()
                    if target_list == False:
                        temp.append(0)
            ...
            for batch in batch_list:
                ...
                for i in range((end_p - start_p).days):
                    ...
                    Raw.sql = "select totalreturn, totalnumber from sklcc_record where batch = '%s' and createtime like '%s%%%%'" % (
                        batch, start_p.date())
                    target_list = Raw.query_all()
                    if target_list == False:
                        temp.append(0)
...
def recheck(request):
    ...
    if 1 in status:
        ...
        Raw = Raw_sql()
        Raw.sql = "select departmentno from sklcc_employee_authority WITH(NOLOCK) where username = '%s' and authorityid = 1" % username
        target_list = Raw.query_all()
        department_list = list()
        for target in target_list:
        ...
        employee_list = []
        Raw.sql = "select distinct username from sklcc_employee_authority WITH(NOLOCK) where authorityid = 0"
        target_list = Raw.query_all()
        ...
        Raw.sql = "select totalnumber, employeeno, employee, questionname, questionno, returnno from sklcc_recheck_bald WITH(NOLOCK) where createtime like '%s%%%%' and recheckor_no = '%s'" % (
            date, em_number)
        target_list = Raw.query_all()
...
def get_size_by_batch(employeeno, batch):
    Raw = Raw_sql()
    styleno = get_styleno_by_batch(batch)
    Raw.sql = """select DISTINCT size from sklcc_style_measure WITH (NOLOCK) where styleno = '%s' and state = 1""" % (
    styleno)
    target_list = Raw.query_all()
    size_list = []
    Raw.sql = "select distinct size, serialno from sklcc_measure_record WITH (NOLOCK) where inspector_no = '%s' and" \
              " createtime like '%s%%%%' and batch = '%s' and is_first_check = 0" % (
              employeeno, Current_time.get_now_date(), batch)
    size_list_scaned = []
    res_list = Raw.query_all()
    if res_list != False:
        for res in res_list:
...
def recheck_update_batch_and_inspector(request):
    ...
    shop3 = []
    choose_list = []
    Raw.sql = "select distinct username, departmentno from sklcc_employee_authority WITH(NOLOCK) where authorityid = 0"
    target_list = Raw.query_all()
    if target_list != False:
        for one in target_list:
    ...
    for month in month_list:
        ...
        if request.GET['is_clear'] == 'true':
            ...
            # temp    = Raw.query_all( 'MSZ' )
            Raw.sql = "select distinct batch from sklcc_record WITH(NOLOCK) where departmentno = '%s' and left( createtime, 7 ) = '%s'" % (
...
def find_department_no(department):
    Raw = Raw_sql()
    Raw.sql = "select departmentno from sklcc_department WITH(NOLOCK) where department = '%s'" % department
    target = Raw.query_one()
    return target[0]
...
def written_into_recheck(recheckinfo):
    ...
    Raw.sql = "insert into sklcc_recheck_content WITH(ROWLOCK) values( '%s', '%s', '%s', %d, %d, %d, '%s', '%s', %d, %d, '%s'," % (
    ...
    if recheckinfo['is_recheck']:
        ...
        Raw.sql += "'False' )"
    Raw.update()
    Raw.sql = "select * from sklcc_recheck_info WITH(NOLOCK) where serialno = '%s'" % recheckinfo['serialno']
    target = Raw.query_one()

    if target == False:
        Raw.sql = "insert into sklcc_recheck_info WITH(ROWLOCK) values( %d, '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s' )" % (
...
def commit_res_recheck(request):
    try:
        ...
        is_return_flag = 0  # 假如有抽验员修改样本量，就需要在更新完sklcc_recheck_content之后，修正sklcc_effciency的值
        SQL = u""
        ...
        leader = ""
        assesstime = ""
        Raw.sql = "select serialno from sklcc_recheck_content WITH(NOLOCK) where contentid = '%s'" % id
        target = Raw.query_one()
        if target != False:
            serialno = target[0]
            SQL += u"delete from sklcc_recheck_content where contentid = '%s';" % id
            Raw.sql = "SELECT assesstime, left(createtime,10) FROM sklcc_recheck_info WHERE serialno = '%s'" % serialno
            target = Raw.query_one()
            if target != False and target[0] != '':
                is_return_flag = 1
        else:
            Raw.sql = "select serialno from sklcc_recheck_info WITH(NOLOCK) where left( createtime, 10 ) = '%s' and batch = '%s' and state = " \
        ...
        if len(record_list) == 0:
            SQL += "insert into sklcc_recheck_content WITH(ROWLOCK) values( '%s', '%s', '%s', %d, %d, %d, '%s', '%s', %d, %d, '%s'," % (
        ...
        if is_new_record_flag:
            SQL += "insert into sklcc_recheck_info WITH(ROWLOCK) " \
                   "values( %d, '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s' );" % (
                       state, serialno, date, departmentno, department, inspector, inspector_no,
                       recheckor, recheckor_no, leader, leader_no, batch, '')

            SQL += "insert into sklcc_return_check WITH(ROWLOCK)( serialno, real_return, ok, state, inner_serialno, questionno ) " \
                   "values( '%s',  %d, %d, %d, '%s', %d )" % (serialno, 0, 1, 0, unicode(uuid.uuid1()), 0)

        Raw.sql = SQL
        Raw.update()
        if is_return_flag == 1:
            written_into_sklcc_effciency(recheckor_no, recheckor, target[1])
        return HttpResponse()
    except Exception, e:
        make_log(sys._getframe().f_code.co_name + ">>>" + str(e))
...
def commit_bald_recheck(request):
    ...
    if len(json['record']) != 0:
        ...
        for record in record_get_list:
            Raw.sql = "insert into sklcc_recheck_bald WITH(ROWLOCK) ( createtime, employeeno, employee, totalnumber, questionname, questionno, returnno, recheckor, recheckor_no )" \
...
def find_recheck_info_help_function(SQL, authorityid_in_use):
    record_list = list()
    record = Record_info()
    Raw = Raw_sql()
    Raw.sql = SQL
    ...
    if target_list != False:
        for target in target_list:
            ...
            record.check_id = 0
            Raw = Raw_sql()
            Raw.sql = "select samplenumber, returnno, questionno from sklcc_recheck_content WITH(NOLOCK) where serialno = '%s'" % serialno
            is_serialno_list = Raw.query_all()
            ...
            for one in is_serialno_list:
                ...
                if get_question_type(one[2]) == 2:
                    ...
                    record.weak += 1
            Raw.sql = "select distinct contentid, samplenumber, totalnumber, is_recheck from sklcc_recheck_content WITH(NOLOCK) where serialno = '%s'" % serialno
            target_list = Raw.query_all()
...
def find_recheck_info(state, username, authorityid_in_use):
    ...
    departmentno_str = ",".join([unicode(departmentno) for departmentno in departmentno_list])
    if authorityid_in_use == 3:
        SQL = "select state,createtime,serialno,departmentno,department,inspector,inspector_no,recheckor,recheckor_no, " \
              "batch from sklcc_recheck_info WITH(NOLOCK)" \
              " where state = %d and recheckor_no = '%s' and createtime > ( SELECT dbo.get_date_before_config_days() )" \
              " and departmentno in( %s ) " % (state, employeeno, departmentno_str)
        ...
        SQL = "select state,createtime,serialno,departmentno,department,inspector,inspector_no,recheckor,recheckor_no," \
              " batch from sklcc_recheck_info WITH(NOLOCK)" \
              " where state = %d and createtime > ( SELECT dbo.get_date_before_config_days() )" \
              " and departmentno in ( %s )" % (state, departmentno_str)
        record_list = find_recheck_info_help_function(SQL, authorityid_in_use)
...
def find_recheck_info_by_date(start, end, username, authorityid_in_use):
    ...
    if authorityid_in_use == 3:
        ...
        for departmentno in departmentno_list:
            SQL = "select state,createtime,serialno,departmentno,department,inspector,inspector_no,recheckor,recheckor_no," \
                  " batch from sklcc_recheck_info WITH(NOLOCK) where state = 2 and recheckor_no = '%s' and departmentno = '%s'" \
        ...
        for departmentno in departmentno_list:
            SQL = "select state,createtime,serialno,departmentno,department,inspector,inspector_no,recheckor, " \
                  "recheckor_no, batch from sklcc_recheck_info WITH(NOLOCK) where state < 2 and departmentno = '%s'" \
                  " and left( createtime,10 ) >= '%s' and left( createtime, 10 ) <= '%s'" % (
...
def delete_recheck_content(request):
    ...
    Raw.sql = "select * from sklcc_recheck_content WITH(NOLOCK) where serialno = '%s'" % serialno
...
def save_recheck_change(xml, serialno):
    ...
    for rc in rc_list:
        ...
        Raw.sql = "select serialno from sklcc_recheck_content WITH(NOLOCK) where contentid = '%s' and workno = %d and questionno = %d" % (
        ...
        if target != False:
            if count == 0:
                ...
                Raw.sql = "update sklcc_recheck_content WITH(ROWLOCK) set returnno = %d where contentid = '%s' and workno = %d and questionno = %d" % (
                    count, id, workno, questionno)
                Raw.update()
        else:
            if count != 0:
                Raw.sql = "select totalnumber, samplenumber from sklcc_recheck_content WITH(NOLOCK) where contentid = '%s'" % id
                target = Raw.query_one()
                ...
                questionname = questionname.decode('gbk')
                Raw.sql = "insert into sklcc_recheck_content WITH(ROWLOCK) values( '%s', '%s', '%s', %d, %d, %d, '%s', '%s', %d, %d, 'False' )" % (
                    id, createtime, serialno, totalnumber, samplenumber, workno, workname, questionname, questionno,
                    count)
                Raw.update()
...
def save_recheck_table(request):
    try:
        ...
        Raw.sql = "select serialno from sklcc_recheck_content WITH(NOLOCK) where contentid = '%s'" % contentid
        target = Raw.query_one()
        serialno = target[0]
        for result in result_list:
            ...
            if ok == '1':
                Raw.sql = "update sklcc_recheck_content WITH(ROWLOCK) set ok = 'True' where contentid = '%s'" % contentid
            else:
                Raw.sql = "update sklcc_recheck_content WITH(ROWLOCK) set ok = 'False' where contentid = '%s'" % contentid
            Raw.update()
...
def update_recheck_info_xml(serialno):
    ...
    Raw.sql = "select distinct contentid from sklcc_recheck_content WITH(NOLOCK) where serialno = '%s'" % serialno
    ...
    Raw.sql = "select department, inspector, inspector_no, batch, createtime from sklcc_recheck_info WITH(NOLOCK) where serialno = '%s'" % serialno
    ...
    for id in id_list:
        Raw.sql = "select distinct workno, workname, totalnumber, samplenumber, ok, is_recheck" \
                  " from sklcc_recheck_content WITH(NOLOCK) where contentid = '%s'" % id
        ...
        for workno in workno_list:
            ...
            Raw.sql = "select questionno, questionname, returnno from sklcc_recheck_content" \
                      " WITH(NOLOCK) where contentid = '%s' and workno = %d" % (
                          id, workno[0])
            target_list = Raw.query_all()
            for target in target_list:
...
def update_recheck_state(request):
    ...
    Raw.sql = "select * from sklcc_recheck_info WITH(NOLOCK) where serialno = '%s'" % serialno
    target = Raw.query_one()
    if target != False:
        Raw.sql = "update sklcc_recheck_info WITH(ROWLOCK) set state = 0 where serialno = '%s'" % serialno
        Raw.update()
    return HttpResponse()
...
def pass_gather_recheck_table(request):
    ...
    Raw.sql = "update sklcc_recheck_info WITH(ROWLOCK) set state = 1 where serialno = '%s'" % serialno
    Raw.update()
    T = Current_time()
    Raw.sql = "update sklcc_recheck_info WITH(ROWLOCK) set leader = '%s', leader_no = '%s', assesstime = '%s'" % (
...
def pass_recheck_table(request):
    ...
    Raw = Raw_sql()
    Raw.sql = "update sklcc_recheck_info WITH(ROWLOCK) set state = %d where serialno = '%s'" % (int(state), serialno)
    Raw.update()
    T = Current_time()
    Raw.sql = "update sklcc_recheck_info WITH(ROWLOCK) set leader = '%s', leader_no = '%s', assesstime = '%s'" % (
        request.session['employee'], request.session['employeeno'], T.time_str)
...
#############################################*** start authority control ***##################################################

# authority_management Tab
def get_em_authority():
    em_authority_list = []
    Raw = Raw_sql()
    Raw.sql = "select employeeno, employee, username from sklcc_employee WITH(NOLOCK)"
    target_list = Raw.query_all()
    ...
    if target_list != False:
        for target in target_list:
            ...
            Raw.sql = "select distinct authorityid from sklcc_employee_authority WITH(NOLOCK) where username = '%s'" % username
            ...
            if authorityid_list != False:
                for authorityid in authorityid_list:
                    ...
                    if (authorityid[0] >= 6 and authorityid[0] <= 14) or authorityid[0] == 16 or authorityid[0] >= 18:
                        ...
                        Raw.sql = "select departmentno from sklcc_employee_authority WITH(NOLOCK)" \
                                  " where username = '%s' and authorityid = %d" % (username, authorityid[0])
                    ...
                    Raw.sql = "select authorityname from sklcc_authority WITH(NOLOCK) where authorityid = %d" % (
                    button.id)
...
def sync_employee_sklcc_record(employee, employeeno):
    Raw = Raw_sql()
    Raw.sql = "update sklcc_record WITH(ROWLOCK) set inspector = '%s' where inspector_no = '%s'" % (
    employee, employeeno)
    Raw.update()
    Raw.sql = "update sklcc_record WITH(ROWLOCK) set assessor = '%s' where assessor_no = '%s'" % (employee, employeeno)
    Raw.update()
    Raw.sql = "update sklcc_employee WITH(ROWLOCK) set employee = '%s' where employeeno = '%s'" % (employee, employeeno)
    Raw.update()
    Raw.sql = "update sklcc_effciency WITH(ROWLOCK) set employee = '%s' where employeeno = '%s'" % (
    employee, employeeno)
    Raw.update()
    Raw.sql = "update sklcc_employeeno_type WITH(ROWLOCK) set employee = '%s' where employeeno = '%s'" % (
    employee, employeeno)
    Raw.update()
    Raw.sql = "update sklcc_employeeno_type WITH(ROWLOCK) set recheckor = '%s' where recheckor_no = '%s'" % (
    employee, employeeno)
    Raw.update()
...
def sync_employee_sklcc_recheck_info(employee, employeeno):
    Raw = Raw_sql()
    Raw.sql = "update sklcc_recheck_info WITH(ROWLOCK) set inspector = '%s' where inspector_no = '%s'" % (
    employee, employeeno)
    Raw.update()
    Raw.sql = "update sklcc_recheck_info WITH(ROWLOCK) set recheckor = '%s' where recheckor_no = '%s'" % (
    employee, employeeno)
    Raw.update()
    Raw.sql = "update sklcc_recheck_info WITH(ROWLOCK) set leader = '%s' where leader_no = '%s'" % (
    employee, employeeno)
    Raw.update()
...
def fix_efficiency(request):
    try:
        start = request.GET['start']
        end   = request.GET['end']
        starty, startm, startd = time.strptime( start, '%Y-%m-%d' )[0:3]
        ...
        for i in range( 0, (end - start).days + 1 ):
            specified_date = (date + datetime.timedelta(i)).strftime('%Y-%m-%d')
            written_into_sklcc_effciency(specified_date=specified_date)
        return HttpResponse(u'全部修正成功！')
    except Exception,e:
        return HttpResponse(u"请检查您输入的链接:"+unicode(str(e)))
...
def get_check_miss_totalnumber(employeeno, date):
    ...
    if target_list != False:
        for target in target_list:
            serialno = target[0]
            Raw.sql = "select distinct contentid, samplenumber from sklcc_recheck_content WITH(NOLOCK) where serialno = '%s'" % serialno
...
def get_recheck_totalnumber(employeeno, date):
    Raw = Raw_sql()
    Raw.sql = "select serialno from sklcc_recheck_info WITH(NOLOCK) where recheckor_no = '%s' and left( createtime, 10 ) = '%s'" % (
        employeeno, date)
...
def get_recheck_serialno_totalreturn(serialno):
    Raw = Raw_sql()
    Raw.sql = "select returnno from sklcc_recheck_content WITH(NOLOCK) where serialno = '%s'" % serialno
...
def get_worktime_by_inspector_date(inspector_no, date, month_or_day):
    ...
    Raw = Raw_sql()
    if month_or_day == 1:
        Raw.sql = """ SELECT sum(worktime), check_type, sum( sumnumber ) from(
                     SELECT check_type, sum(totalnumber) as sumnumber,
                      batch, dbo.get_worktime_by_batch_and_checktype(batch,check_type)  * sum(totalnumber)  as worktime
                      FROM sklcc_record  WITH(NOLOCK)
                      WHERE left(createtime,10) = '%s'
                      AND inspector_no = '%s' GROUP BY check_type,batch
                    ) as table_temp
                    group by check_type"""% ( date,inspector_no )
        target_list = Raw.query_all()
        column_list = []
        res_list = []
        if target_list != False:
            target_dynamic_check_type = list(set( [target[1] for target in target_list] ))
            res_list = [ 0 for i in range(0, len(target_dynamic_check_type)*2 + 2) ]
            for check_type in target_dynamic_check_type:
                column_list.append( check_type + u"总数")
                column_list.append( check_type + u"工时")
            for target in target_list:
                res_list[target_dynamic_check_type.index( target[1] )*2]   = target[2]
                res_list[target_dynamic_check_type.index( target[1] )*2+1] = target[0]
                res_list[-1] += target[0] #叠加今日检验工时
                res_list[-2] += target[2] #叠加今日检验总数
        return res_list, column_list
    else:
        Raw.sql = "SELECT check_type, sum(totalnumber), batch, dbo.get_worktime_by_batch_and_checktype(batch,check_type) * sum(totalnumber)" \
                  " FROM sklcc_record  WITH(NOLOCK)" \
                  " WHERE createtime >= '%s' AND createtime <= '%s' AND inspector_no = '%s'" \
                  " GROUP BY check_type, batch"% ( date[0:8] + '01', date[0:8] + '31',inspector_no)
        target_list = Raw.query_all()
        column_list = []
        res_list = {}
        if target_list != False:
            target_dynamic_check_type = list(set( [target[0] for target in target_list] ))
            for check_type in target_dynamic_check_type:
                column_list.append( check_type + u"总数")
                column_list.append( check_type + u"工时")

            for target in target_list:
                if target[2] not in res_list.keys():
                    res_list[target[2]] = [ 0 for i in range(0 , len(target_dynamic_check_type)*2 + 2)] #加2是增加两列：总件数和总工时
                    res_list[target[2]][target_dynamic_check_type.index(target[0])*2]   = target[1]
                    res_list[target[2]][target_dynamic_check_type.index(target[0])*2+1] = target[3]
                    res_list[target[2]][-2] = target[1] #叠加今日检验总数
                    res_list[target[2]][-1] = target[3] #叠加今日检验工时
                else:
                    res_list[target[2]][target_dynamic_check_type.index(target[0])*2]   = target[1]
                    res_list[target[2]][target_dynamic_check_type.index(target[0])*2+1] = target[3]
                    res_list[target[2]][-2] += target[1] #叠加今日检验总数
        ...
        return res_list,column_list, len(column_list) + 3
...
def myinfo(request):
    ...
    worktime_list, day_column_list           = get_worktime_by_inspector_date(employeeno, today, 1)
    worktime_month_list, month_column_list, month_col_len = get_worktime_by_inspector_date(employeeno, today, 0)

    today_total_check    = worktime_list[-2] if len(worktime_list) > 0 else 0
    today_total_worktime = worktime_list[-1] if len(worktime_list) > 0 else 0
    worktime_list        = worktime_list[:-2]

    sum_month_list = [ 0 for i in range(0, len( month_column_list )+2) ]
    for batch,worktime in worktime_month_list.items():
    ...
    if target_list != False:
        ...
        for one in target_list:
            if one[4] not in assesstime_list:
                ...
                temp_son = {'assesstime': one[4].split(' ')[0] if one[4] != None else 0, 'inner_serialno': one[0],
                            'res': [], 'ok': one[8], 'real': one[9],
    ...
    try:
        ...
        for one in res:
            ...
            if len(question_list) == 0:
                if totalreturn < 0:
                    Raw.sql = "insert into sklcc_return_check WITH(ROWLOCK) ( serialno, real_return, ok, state, inner_serialno, questionno, assesstime, returnno )" \
                              " values( '%s',  %d, %d, %d, '%s', %d, '%s', %d )" % (
                              serialno, realreturn, ok, 1, inner_serialno, 0, T.time_str, 0)
                else:
                    Raw.sql = "insert into sklcc_return_check WITH(ROWLOCK) ( totalreturn, serialno, real_return, ok, state, inner_serialno, questionno, assesstime, returnno )" \
                              " values( %d, '%s',  %d, %d, %d, '%s', %d, '%s', %d )" % (
                              totalreturn, serialno, realreturn, ok, 1, inner_serialno, 0, T.time_str, 0)
                Raw.update()
            else:
                Raw.sql = ""
                if totalreturn < 0:
                    for question in question_list:
                        ...
                        Raw.sql += "insert into sklcc_return_check WITH(ROWLOCK) ( serialno, real_return, ok, state, questionname, questionno, returnno, " \
                    ...
                    for question in question_list:
                        ...
                        questionno = int(question['no'])
                        returnno = int(question['count'])
                        Raw.sql += "insert into sklcc_return_check WITH(ROWLOCK) ( totalreturn, serialno, real_return, ok, state, questionname, questionno, returnno, " \
                                   "assesstime, inner_serialno )" \
